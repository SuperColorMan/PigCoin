...
class ShowContentPage extends StatefulWidget {
  ...
  ShowContentPage(this._contentInfo);

  @override
  _ShowContentPageState createState() => _ShowContentPageState(_contentInfo);
}
...
class _ShowContentPageState extends State<ShowContentPage> {
  ...
  _ShowContentPageState(this._contentInfo);
...
//    _refresh();
  ...
  void _initPage(StateSetter setState) {
    ...
    ///  -------------------- 初始化关联商品 start --------------------
    List<Widget> relCommodityCardList = [];
    Future<Map<String, dynamic>> relCommodityResMap =
        _globalNetApiCall.getRelCommodityByContentId(_contentId.toString());
    relCommodityResMap.then((value) {
      int code = value['code'];
      if (code == 0) {
        List<Map> _relCommodityList = value['data'].cast<Map>();
        if (_relCommodityList.length > 0) {
          for (Map commodty in _relCommodityList) {
            relCommodityCardList.add(CommodityCard_1(commodty));
            print("评论初始化内容加入的话题列表失败-----${commodty.toString()}");
          }
          relCommodityArea = Container(
              ...
              children: [
                ...
                Container(
                  ...
                  child: SingleChildScrollView(
                    scrollDirection: Axis.horizontal,
                    child: Row(children: relCommodityCardList),
                  ),
                ),
              ],
            ),
